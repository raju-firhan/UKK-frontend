...
export default function Menu() {
    ...
    useEffect(() => {
        const fecthAllMenu = async () => {
            try {
                const res = await axios.get("http://localhost:8000/menu/", {headers})
                console.log(res)
    ...
    const deleteId = async () => {
        try {
            await axios.delete("http://localhost:8000/menu/" + pickId, {headers})
                            ...
                            <tbody>
                                {menu && menu.map((menu, index) => (
                                    <tr key={menu.id} className="bg-white border-b hover:bg-gray-50">
                                        <td>
                                            <img className="h-auto max-w-[100%]" src={`http://localhost:8000/foto/${menu.gambar}`} alt="product" />
                                        </td>
